...
CLASS zcl_ea_salv_table_ext_row_info DEFINITION PUBLIC INHERITING FROM zcl_ea_salv_table CREATE PUBLIC.
  PUBLIC SECTION.
    TYPES:
...
CLASS zcl_ea_salv_table_ext_row_info IMPLEMENTATION.
  METHOD set_data.
    super->set_data( create_table_copy = create_table_copy data_table = data_table ).

    "color col
    TRY.
        me->columns->set_as_color( c_col-color ).
        is_col_present-color = abap_true.
      CATCH cx_salv_data_error.
        is_col_present-color = abap_false.
    ENDTRY.

    "success col
    TRY.
        me->columns->get( c_col-success ).
        is_col_present-success = abap_true.
      CATCH cx_salv_not_found.
        is_col_present-success = abap_false.
    ENDTRY.
  ENDMETHOD.
